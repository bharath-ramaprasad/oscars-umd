package net.es.oscars.pce.bandwidth.common;

import static java.util.Arrays.asList;

import net.es.oscars.logging.OSCARSNetLogger;
import net.es.oscars.pce.bandwidth.BandwidthPCEServer;
...
public class Invoker {
    ...
    public static void main(String[] args) throws Exception {
        ...
        parseArgs( args );
        System.out.println("starting bandwidthPCE service  with context " + context);
        cc.setContext(context);
        cc.setServiceName(ServiceNames.SVC_PCE_BW);
        try {
        ...
        BandwidthPCEServer server = new BandwidthPCEServer();
        server.startServer(false);
        log.info(netLogger.end("invoker","bandwidth PCE server has started with context: " + context));
