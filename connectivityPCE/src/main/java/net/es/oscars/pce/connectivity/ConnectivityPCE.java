...
public class ConnectivityPCE {
    ...
    public PCEDataContent calculatePath(PCEMessage query) throws OSCARSServiceException{
        ...
        */
        ...
        /*
        List<OptionalConstraintType> optConstraints = pceData.getOptionalConstraint();
        //if(optConstraints.get(0).getCategory().equals("anycastDests")){
        	List<Object> dstList = optConstraints.get(0).getValue().getAny();
        //}
         */
        ...
        for(int i = 0; i < dstList.size(); i++){
    		...
    		/*JAXBElement endpointTag = (JAXBElement)(dstList.get(i));
    		this.log.debug("MB endpointTag is: " + endpointTag.getName());
    		pceData.getUserRequestConstraint().getPathInfo().getLayer2Info().setDestEndpoint((JAXBElement)endpointTag.getValue());
			*/
