...
public class PathComputationMutex {
    ...
    public synchronized void get (CoordRequest request) throws InterruptedException {
        ...
        LOG.debug(netLogger.start(event, request.getName() + 
    ...
    public synchronized void release(String gri) {
        ...
        if ( ! gri.equals(this.holdingGRI) && (this.holdingGRI != null)) {
            // do nothing. 
            LOG.debug(netLogger.error(event,ErrSev.MINOR,"cannot release: wrong GRI"));
            return;
        }
        if (this.locked) {
            this.locked = false;
