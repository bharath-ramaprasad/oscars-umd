...
@OSCARSService (
        implementor = "net.es.oscars.pce.nullagg.NullAggProtocolHandler",
...
public class NullAgg extends PCEProtocolServer implements Runnable {
    ...
    private void processQuery (AggMessage query) throws OSCARSServiceException {
        ...
        try {
            ...
            if (query.getMethod().equals("pceCreate")) {
                client.sendPceReply(query.getMessageProperties(),query.getGri(), query.getTransactionId(), query.getPceName(), pceData);
            } else if (query.getMethod().equals("pceCommit")) {
                client.sendCommitReply(query.getMessageProperties(),query.getGri(), query.getTransactionId(), query.getPceName(), pceData);
            } else if (query.getMethod().equals("pceCancel")) {
                client.sendCancelReply(query.getMessageProperties(),query.getGri(), query.getTransactionId(), query.getPceName(), pceData, null);
            } else if (query.getMethod().equals("pceModify")) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            parseArgs(args);
            System.out.println("starting null Aggregator service with context "+ context);
