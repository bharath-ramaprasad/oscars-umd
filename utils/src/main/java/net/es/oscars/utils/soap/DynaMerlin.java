...
public class DynaMerlin extends Merlin {
     ...
     * load the propterties from the property file in the service configuration directory.
     * deployProps has properties that can be modified by a deployment site.
     * @param service unqualified service name
     * @param file unqualified property file name
     */
    private void readProps (String service, String file) {

        this.deployProps = new Properties();
        try {
            String pathname = (new SharedConfig(service)).getFilePath(file);
    ...
    private void setProps () throws IOException {
        ...
        String service  = this.properties.getProperty ("net.es.oscars.utils.soap.DynaMerlin.service");
        this.readProps (service, propFile);
