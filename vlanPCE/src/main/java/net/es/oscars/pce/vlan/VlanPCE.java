...
public class VlanPCE {
    ...
    public PCEDataContent calculatePath(PCEMessage query, OSCARSNetLogger netLogger) throws OSCARSServiceException, MalformedURLException{
        ...
        */
		...
		/*************************************************************
						UMassD Anycast Implementation
			Author: 	Tim Entel (tentel@umassd.edu)
						Jeremy Plante (jplante@umassde.edu)
			Date:		7-19-2011
			Version:	0.1
			Comment:	The reqElemMap contains data concerning the
						destination.  Our implementation loops for
						each destination in order to build a map which 
		...
		************************************************************/
            ...
            for(CtrlPlaneHopContent hop : resv.getReservedConstraint().getPathInfo().getPath().getHop()){
                ...
                try{
                    ...
                    if(!vlanMap.containsKey(urnKey)){
                        vlanMap.put(urnKey, new VlanRange());
                    }
        ...
        for(CtrlPlaneDomainContent domain : inputTopo.getDomain()){
            if(!this.localDomain.equals(NMWGParserUtil.normalizeURN(domain.getId()))){
                continue;
            }
                ...
                for(CtrlPlanePortContent port : node.getPort()){
                    ...
                    for(CtrlPlaneLinkContent link : port.getLink()){
						...
						/*************************************************************
							...
							Date:		7-15-2011
							Version:	0.1
							Comment:	The method isEdge is changed to accept the set
										of possible destinations, rather than only a 
										single one.
						************************************************************/		
								
                        if(!this.isEdge(link, src, dstList) && (!this.isVlanLink(link) || this.mpls)){
                            //clear out vlans
    ...
    private boolean isEdge(CtrlPlaneLinkContent link, String src, ArrayList<String> dest) throws OSCARSServiceException {
		...
		/*************************************************************
			...
			Date:		7-15-2011
			Version:	0.1
			Comment:	Check to see if the linkID matches any of the
						possible destinations
		************************************************************/
		for( int i = 0; i < dest.size(); i++){
			if(linkId.equals(dest.get(i))){
				return true;
			}
		}
        return false;
